<?php
session_start();
require_once "includes/bloodbankfiles/dbconfig.php";
if(!isset($_SESSION["bloodbankpasswordaccess"]) || $_SESSION["bloodbankpasswordaccess"] !== true){
  ...
  if(isset($_POST['resetpassword'])){
    ...
    $bloodbankconfirmpassword =$_POST['bloodbankconfirmpassword'];
    $useremail =$_SESSION['bloodbankemail'];

   if(empty($bloodbankpassword) && empty($bloodbankconfirmpassword)){
           ...
           if(strlen($bloodbankpassword)>2){
              if($bloodbankpassword == $bloodbankconfirmpassword){
                   ...
                   while($passwordfetch =mysqli_fetch_assoc($querypasswordcheck)){
                      if(password_verify($bloodbankpassword, $passwordfetch['userpassword'])){
                          $_SESSION['resetpassworderror']="please use another password";
                      }else{
                          $resethashpassword = password_hash($bloodbankpassword, PASSWORD_DEFAULT);
                          ...
                          if($querypasswordupdate){
                               $querypasswordupdatebloodbank=mysqli_query($dlink,"UPDATE bloodbankregister SET bloodbankpassword=' $resethashpassword' WHERE bloodbankemail='$useremail'");
                               if($querypasswordupdatebloodbank){
                                   session_destroy();
           ...
           <div class="card shadow-lg">
                ...
                <form  method="post">
                    <div class="form-items position-relative">
                        <input type="password" name="bloodbankpassword" id="" class="form-control rounded-pill" placeholder="Enter New Password">
                    ...
                    <div class="form-items position-relative">
                        <input type="password" name="bloodbankconfirmpassword" id="" class="form-control rounded-pill" placeholder="Enter Confirm Password">
                        <span class="passwordEye"><i class="fa-solid fa-eye"></i></span>
                        <?php
                ...
                <hr class="mx-2">
                <a href="bloodbankforgetpassword?outpage" class="text-center text-decoration-none">Back To Home</a>
           </div>
      </div>
  </div>
</body>
