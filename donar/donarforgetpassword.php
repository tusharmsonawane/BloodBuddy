...
require_once "includes/donarfiles/dbconfig.php";
if(!isset($_SESSION["donarpasswordaccess"]) || $_SESSION["donarpasswordaccess"] !== true){
  ...
  if(isset($_POST['resetpassword'])){
      ...
      $donarconfirmpassword =$_POST['donarconfirmpassword'];
      $useremail =$_SESSION['donaremail'];

     if(empty($donarpassword) && empty($donarconfirmpassword)){
        ...
        if(empty($donarpassword)){
           ...
           if(empty($donarconfirmpassword)){
            $_SESSION['resetpassworderror2']="please enter the confirm password";
           }else{
             if(strlen($donarpassword)>2){
                if($donarpassword == $donarconfirmpassword){
                     ...
                     while($passwordfetch =mysqli_fetch_assoc($querypasswordcheck)){
                        if(password_verify($donarpassword, $passwordfetch['userpassword'])){
                            $_SESSION['resetpassworderror']="please use another password";
                        }else{
                            $resethashpassword = password_hash($donarpassword, PASSWORD_DEFAULT);
                            ...
                            if($querypasswordupdate){
                                 $querypasswordupdatedonar=mysqli_query($dlink,"UPDATE donarregister SET donarpassword=' $resethashpassword' WHERE donaremail='$useremail'");
                                 if($querypasswordupdatedonar){
                                     session_destroy();
           ...
           <div class="card shadow-lg">
                ...
                <form  method="post">
                    <div class="form-items position-relative">
                        <input type="password" name="donarpassword" id="" class="form-control rounded-pill" placeholder="Enter New Password">
                    ...
                    <div class="form-items position-relative">
                        <input type="password" name="donarconfirmpassword" id="" class="form-control rounded-pill" placeholder="Enter Confirm Password">
                        <span class="passwordEye"><i class="fa-solid fa-eye"></i></span>
                            <?php
                ...
                <hr class="mx-2">
                <a href="donarforgetpassword?outpage" class="text-center text-decoration-none">Back To Home</a>
           </div>
      </div>
  </div>
</body>
